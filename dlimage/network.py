...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta, test_data=None):
        """Train the neural network using mini-batch stochastic gradient descent.  The raining_data is a
    ...
    def update_mini_batch(self, mini_batch, eta):
        """ Update the network's weights and biases by applying gradient descent using back-propagation to
           a single mini batch. The mini_batch is a list of tuples (x, y), and eta is the learning rate."""
    ...
    def backprop(self, x, y):
        ...
        for layer in range(2, self.num_layers):
            ...
            delta = np.dot(self.weights[-layer + 1].transpose(), delta) * sd  # L - layer + 1
            nabla_b[-layer] = delta
            nabla_w[-layer] = np.dot(delta, activations[-layer - 1].transpose())  # L-  layer - 1
        return nabla_b, nabla_w

    def cost_derivative(self, output_activations, y):
        """Return the vector of partial derivatives of C_x to a for the output activations."""
